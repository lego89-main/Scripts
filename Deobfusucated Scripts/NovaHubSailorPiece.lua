--FH
if getgenv().NovaHub_Running then
...
local function PostToWebhook()
    ...
    local payload = {
        ["embeds"] = {{
            ...
            ["footer"] = { ["text"] = string.format("NovaHub â€¢ Session: %s â€¢ %s", GetSessionTime(), os.date("%x %X")) },
...
local function SendSafetyWebhook(targetPlayer, reason)
    ...
    local payload = {
        ["embeds"] = {{
            ...
            ["footer"] = { ["text"] = "NovaHub â€¢ " .. os.date("%x %X") }
        }}
    }
            ...
            if url ~= "" and url:find("discord.com/api/webhooks/") then
                local payload = {
                    ["embeds"] = {{
                        ...
                        ["footer"] = { ["text"] = "NovaHub | Sailor Piece" }
                    }}
                }
                task.spawn(function()
                    pcall(function()
...
task.spawn(function()
    while task.wait(5) do
        if not getgenv().NovaHub_Running then break end
...
task.spawn(function()
    while getgenv().NovaHub_Running do
...
ThemeManager:SetFolder("NovaHub | Sailor Piece")
SaveManager:SetFolder("NovaHub/SailorPiece")
