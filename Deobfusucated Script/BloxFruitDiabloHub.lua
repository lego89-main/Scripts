...
_G.Title = "R2lx Hub Script blox fruits" -- คำอธิบาย
-----------------------------------------------------------------
...
ImageButton.Position = UDim2.new(0.02, 0, 0.85, 0)
ImageButton.Size = UDim2.new(0, 50, 0, 50)
ImageButton.Draggable = true
ImageButton.Image = "rbxthumb://type=Asset&id=113151220863963&w=150&h=150"
ImageButton.ImageColor3 = Color3.fromRGB(255, 255, 255) -- White, no green tint
ImageButton.AutoButtonColor = false

UICorner.CornerRadius = UDim.new(0.2, 0)
UICorner.Parent = ImageButton

UIStroke.Color = Color3.fromRGB(180, 180, 190) -- Simple gray border
UIStroke.Thickness = 1.5
UIStroke.Parent = ImageButton

-- Hover effects (simple, no green)
ImageButton.MouseEnter:Connect(function()
    game:GetService("TweenService"):Create(ImageButton, TweenInfo.new(0.2), {
        BackgroundColor3 = Color3.fromRGB(40, 40, 50),
        Size = UDim2.new(0, 54, 0, 54)
    }):Play()
    UIStroke.Color = Color3.fromRGB(255, 50, 50) -- Red border on hover
end)
...
ImageButton.MouseLeave:Connect(function()
    ...
    UIStroke.Color = Color3.fromRGB(180, 180, 190) -- Back to gray
end)
...
ImageButton.MouseButton1Click:Connect(function()
    ...
    toggleUI()
    -- Click feedback (red flash)
    game:GetService("TweenService"):Create(ImageButton, TweenInfo.new(0.1), {
...
function library:NaJa()
	...
	local Accent = _G.Color or Color3.fromRGB(108, 192, 255)

	-- ── Palette ───────────────────────────────────────────────
	local BG      = Color3.fromRGB(9,  10, 14)   -- main bg
	local Surface = Color3.fromRGB(13, 15, 21)   -- topbar / sidebar
	local Card    = Color3.fromRGB(16, 19, 26)   -- section cards
	local Raised  = Color3.fromRGB(21, 25, 34)   -- inputs / toggles / buttons
	local Border  = Color3.fromRGB(30, 35, 50)   -- all borders
	local TxtPri  = Color3.fromRGB(228, 232, 242)
	local TxtMut  = Color3.fromRGB(100, 110, 132)

	local function Tween(obj, props, dur, style, dir)
		if not obj then return end
		local info = TweenInfo.new(dur or 0.22, style or Enum.EasingStyle.Quart, dir or Enum.EasingDirection.Out)
	...
	local function fitText(label, text, maxSize, minSize)
		...
		for size = maxSize, minSize, -1 do
			local bounds = TextService:GetTextSize(text, size, label.Font, Vector2.new(10000, 1000))
			if bounds.X <= width then chosen = size; break end
		end
	...
	function applyUISize(size)
		...
		local w, h = s.X, s.Y
		if size == "Auto" then
			if     w <= 600 or h <= 400 then Main.Size = UDim2.new(0, 480, 0, 320)
			elseif w <= 800 or h <= 600 then Main.Size = UDim2.new(0, 520, 0, 360)
			elseif w <= 1100 or h <= 700 then Main.Size = UDim2.new(0, 580, 0, 400)
			elseif w <= 1400 or h <= 900 then Main.Size = UDim2.new(0, 640, 0, 450)
			else                              Main.Size = UDim2.new(0, 720, 0, 500) end
		elseif size == "Large"  then Main.Size = UDim2.new(0, 720, 0, 500)
		elseif size == "Big"    then Main.Size = UDim2.new(0, 640, 0, 450)
		elseif size == "Normal" then Main.Size = UDim2.new(0, 520, 0, 360) end
		Main.Position = UDim2.new(0.5, 0, 0.5, 0)
	end
	local function setupMobile()
		if _G.UISize == "Auto" then applyUISize("Auto") end
	end
	setupMobile()
	workspace.CurrentCamera:GetPropertyChangedSignal("ViewportSize"):Connect(setupMobile)
	applyUISize("Auto")
	...
	-- fill the lower-half corners so they're square against content
	local TopBarFix = Instance.new("Frame")
	TopBarFix.Parent = TopBar
	TopBarFix.BackgroundColor3 = Surface
	...
	TopBarLine.BackgroundColor3 = Border
	TopBarLine.BackgroundTransparency = 0
	TopBarLine.BorderSizePixel = 0
	TopBarLine.Position = UDim2.new(0, 0, 1, -1)
	TopBarLine.Size = UDim2.new(1, 0, 0, 1)

	-- Brand accent mark (thin vertical pill)
	local BrandMark = Instance.new("Frame")
	BrandMark.Parent = TopBar
	BrandMark.BackgroundColor3 = Accent
	BrandMark.BorderSizePixel = 0
	BrandMark.Position = UDim2.new(0, 16, 0.5, -8)
	BrandMark.Size = UDim2.new(0, 2, 0, 16)
	local BrandMarkCorner = Instance.new("UICorner")
	BrandMarkCorner.CornerRadius = UDim.new(1, 0)
	BrandMarkCorner.Parent = BrandMark

	local Title = Instance.new("TextLabel")
	Title.Name = "Title"
	Title.Parent = TopBar
	Title.BackgroundTransparency = 1
	Title.Position = UDim2.new(0, 26, 0, 9)
	Title.Size = UDim2.new(0, 200, 0, 16)
	Title.Font = Enum.Font.GothamBold
	Title.Text = "Diablo Hub"
	Title.TextColor3 = TxtPri
	Title.TextSize = 13
	...
	Desc.Position = UDim2.new(0, 26, 0, 28)
	Desc.Size = UDim2.new(0, 200, 0, 12)
	Desc.Font = Enum.Font.Gotham
	Desc.Text = "Made by Diablo"
	Desc.TextColor3 = TxtMut
	Desc.TextSize = 10
	Desc.TextXAlignment = Enum.TextXAlignment.Left

	-- Shared helper: clean header button
	local function makeHeaderBtn(parent, xOffset, w, iconId, label, zBase)
		...
		Icon.Position = UDim2.new(0, 7, 0.5, -6)
		Icon.Size = UDim2.new(0, 12, 0, 12)
		Icon.Image = iconId
		Icon.ImageColor3 = TxtMut
		Icon.ZIndex = zBase + 1
		...
		Lbl.Text = label
		Lbl.TextColor3 = TxtMut
		Lbl.TextSize = 11
		Lbl.ZIndex = zBase + 1

		Btn.MouseEnter:Connect(function()
			Tween(Btn, { BackgroundColor3 = Color3.fromRGB(27, 32, 44) }, 0.14)
			Tween(Icon, { ImageColor3 = TxtPri }, 0.14)
			Tween(Lbl, { TextColor3 = TxtPri }, 0.14)
		end)
		Btn.MouseLeave:Connect(function()
			Tween(Btn, { BackgroundColor3 = Raised }, 0.14)
			Tween(Icon, { ImageColor3 = TxtMut }, 0.14)
			Tween(Lbl, { TextColor3 = TxtMut }, 0.14)
		end)
		return Btn, Icon, Lbl, BtnStroke
	end

	-- Discord button
	local Discord, DiscIcon, DiscText = makeHeaderBtn(
		TopBar, -228, 100, "rbxassetid://12058969086", "Discord"
	)
	Discord.MouseButton1Click:Connect(function()
		if setclipboard then setclipboard("https://discord.gg/SpJQ3XVEJ8") end
		if toclipboard then toclipboard("https://discord.gg/SpJQ3XVEJ8") end
	...
	local SizeDropdown, SizeIcon, SizeText = makeHeaderBtn(
		TopBar, -122, 106, "rbxassetid://7072718266", "Auto", 200
	)

	-- Size panel (floats above main)
	local SizePanel = Instance.new("Frame")
	SizePanel.Name = "SizePanel"
	SizePanel.Parent = Main
	SizePanel.BackgroundColor3 = Card
	SizePanel.BackgroundTransparency = 0
	SizePanel.BorderSizePixel = 0
	SizePanel.Position = UDim2.new(1, -130, 0, 54)
	SizePanel.Size = UDim2.new(0, 112, 0, 0)
	SizePanel.ClipsDescendants = true
	SizePanel.ZIndex = 300

	local SizePanelCorner = Instance.new("UICorner")
	SizePanelCorner.CornerRadius = UDim.new(0, 6)
	SizePanelCorner.Parent = SizePanel

	local SizePanelStroke = Instance.new("UIStroke")
	SizePanelStroke.Color = Border
	SizePanelStroke.Thickness = 1
	SizePanelStroke.Parent = SizePanel

	local SizePanelPad = Instance.new("UIPadding")
	SizePanelPad.Parent = SizePanel
	SizePanelPad.PaddingLeft = UDim.new(0, 4)
	SizePanelPad.PaddingRight = UDim.new(0, 4)
	SizePanelPad.PaddingTop = UDim.new(0, 4)
	SizePanelPad.PaddingBottom = UDim.new(0, 4)

	local SizeListLayout = Instance.new("UIListLayout")
	SizeListLayout.Parent = SizePanel
	...
	for i, option in ipairs(sizeOptions) do
		local btn = Instance.new("TextButton")
		btn.Name = option .. "Btn"
		btn.Parent = SizePanel
		btn.BackgroundColor3 = option == "Auto" and Accent or Card
		btn.BackgroundTransparency = option == "Auto" and 0 or 1
		btn.BorderSizePixel = 0
		btn.Size = UDim2.new(1, 0, 0, 28)
		btn.AutoButtonColor = false
		btn.Font = Enum.Font.Gotham
		btn.Text = option
		btn.TextColor3 = option == "Auto" and Color3.fromRGB(8, 10, 14) or TxtMut
		btn.TextSize = 11
		btn.ZIndex = 301
		...
		btn.MouseEnter:Connect(function()
			if option ~= _G.UISize then
				btn.BackgroundColor3 = Raised
				Tween(btn, { BackgroundTransparency = 0, TextColor3 = TxtPri }, 0.12)
			end
		end)
		btn.MouseLeave:Connect(function()
			if option ~= _G.UISize then
				btn.BackgroundColor3 = Card
				Tween(btn, { BackgroundTransparency = 1, TextColor3 = TxtMut }, 0.12)
			end
		end)
		btn.MouseButton1Click:Connect(function()
			_G.UISize = option
			SizeText.Text = option
			for _, b in pairs(sizeButtons) do
				if b.Text == option then
					b.BackgroundColor3 = Accent
					Tween(b, { BackgroundTransparency = 0, TextColor3 = Color3.fromRGB(8, 10, 14) }, 0.15)
				else
					b.BackgroundColor3 = Card
					Tween(b, { BackgroundTransparency = 1, TextColor3 = TxtMut }, 0.15)
				end
			end
			applyUISize(option)
			panelOpen = false
	...
	SizeDropdown.MouseButton1Click:Connect(function()
		...
		if panelOpen then
			SizePanel.Size = UDim2.new(0, 112, 0, 0)
			Tween(SizePanel, { Size = UDim2.new(0, 112, 0, #sizeOptions * 30 + 8) }, 0.2)
		else
			Tween(SizePanel, { Size = UDim2.new(0, 112, 0, 0) }, 0.18)
		end
	end)
	...
	function tabs:Tab(Name, icon)
		...
		function sections:Section(Name, side)
			...
			function functionitem:Toggle(Name, default, callback)
				...
				SwitchBg.BackgroundColor3 = Color3.fromRGB(32, 38, 52)
				...
				SwitchKnob.BackgroundColor3 = Color3.fromRGB(170, 175, 192)
				...
				local function updateToggleText()
					task.defer(function() fitText(ToggleText, tostring(Name), 12, 9) end)
				end
				updateToggleText()
				ToggleText:GetPropertyChangedSignal("AbsoluteSize"):Connect(updateToggleText)

				local function UpdateToggle(state)
					Tween(SwitchBg, { BackgroundColor3 = state and Accent or Color3.fromRGB(32, 38, 52) }, 0.2)
					Tween(SwitchKnob, {
						Position = state and UDim2.new(0, 23, 0.5, -6) or UDim2.new(0, 3, 0.5, -6),
						BackgroundColor3 = state and Color3.fromRGB(255, 255, 255) or Color3.fromRGB(170, 175, 192)
					}, 0.2)
					Tween(ToggleText, { TextColor3 = state and TxtPri or TxtMut }, 0.2)
					callback(state)
					Tgo = state
				end
				...
				ToggleBtn.MouseButton1Click:Connect(function()
					Tgo = not Tgo
					UpdateToggle(Tgo)
					if CircleClick then CircleClick(ToggleFrame, Mouse.X, Mouse.Y) end
				end)
				ToggleBtn.MouseEnter:Connect(function()
					Tween(ToggleStroke, { Color = Color3.fromRGB(48, 56, 74) }, 0.14)
				end)
				ToggleBtn.MouseLeave:Connect(function()
					Tween(ToggleStroke, { Color = Border }, 0.14)
				end)

				if default then UpdateToggle(default) end

				function ToglFunc:Update(state) UpdateToggle(state) end
			...
			function functionitem:Textbox(Name, Placeholder, callback)
				...
				TextboxFrame.BackgroundColor3 = Raised
				...
				TBLabel.Position = UDim2.new(0, 12, 0, 7)
				TBLabel.Size = UDim2.new(1, -16, 0, 12)
				TBLabel.Font = Enum.Font.Gotham
				TBLabel.Text = string.upper(Name)
				TBLabel.TextColor3 = TxtMut
				TBLabel.TextSize = 9
				TBLabel.TextXAlignment = Enum.TextXAlignment.Left

				local InputBg = Instance.new("Frame")
				InputBg.Parent = TextboxFrame
				InputBg.BackgroundColor3 = BG
				...
				InputBox.Name = "HeadTitle"
				InputBox.Parent = InputBg
				InputBox.BackgroundTransparency = 1
				InputBox.Size = UDim2.new(1, -10, 1, 0)
				InputBox.Position = UDim2.new(0, 5, 0, 0)
				InputBox.Font = Enum.Font.Gotham
				InputBox.PlaceholderColor3 = Color3.fromRGB(58, 66, 84)
				InputBox.PlaceholderText = Placeholder or "Enter value..."
				InputBox.Text = ""
				InputBox.TextColor3 = TxtPri
				InputBox.TextSize = 11
				InputBox.TextXAlignment = Enum.TextXAlignment.Left

				InputBox.Focused:Connect(function()
					Tween(TBStroke, { Color = Accent }, 0.15)
				end)
				InputBox.FocusLost:Connect(function(enterPressed)
					Tween(TBStroke, { Color = Border }, 0.15)
			...
			local function buildDropHeader(parent)
				...
				DisplayText.Size = UDim2.new(1, -30, 0, 32)
				DisplayText.ZIndex = 16
				DisplayText.Font = Enum.Font.Gotham
				DisplayText.Text = ""
				DisplayText.TextColor3 = TxtMut
				DisplayText.TextXAlignment = Enum.TextXAlignment.Left
				DisplayText.ClipsDescendants = true
    			...
    			Arrow.ImageColor3 = TxtMut

				local Scroll = Instance.new("ScrollingFrame")
				Scroll.Name = "Scroll_Items"
				Scroll.Parent = Frame
				Scroll.Active = true
				...
				return Frame, HitBtn, DisplayText, Arrow, Scroll, SList, FStroke
			end
			...
			local function makeDropItem(parent, text, isSelected)
				...
				item.TextColor3 = isSelected and Accent or TxtMut
				item.TextSize = 11
				...
				item.MouseEnter:Connect(function() Tween(item, { BackgroundColor3 = Raised }, 0.12) end)
				item.MouseLeave:Connect(function() Tween(item, { BackgroundColor3 = BG }, 0.12) end)
				return item
			end
			...
			function functionitem:Dropdown(Name, list, default, callback)
				...
				local Frame, HitBtn, DisplayText, Arrow, Scroll, SList, FStroke =
					buildDropHeader(SectionContainer)

				local function refreshCanvas()
					Scroll.CanvasSize = UDim2.new(0, 0, 0, SList.AbsoluteContentSize.Y + 8)
				end
				SList:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(refreshCanvas)

				local function updateText()
					if selected then
						DisplayText.Text = Name .. "  ›  " .. tostring(selected)
				...
				task.defer(function()
					updateText()
					DisplayText:GetPropertyChangedSignal("AbsoluteSize"):Connect(updateText)
				end)
				...
				function Dropfunc:TogglePanel(state)
					...
					TweenService:Create(Frame, TweenInfo.new(0.2, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {
						Size = Tof and UDim2.new(1, 0, 0, math.clamp(40 + SList.AbsoluteContentSize.Y + 8, 32, 220)) or UDim2.new(1, 0, 0, 32)
					}):Play()
					TweenService:Create(Arrow, TweenInfo.new(0.2, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), { Rotation = Tof and 180 or 0 }):Play()
					Tween(FStroke, { Color = Tof and Accent or Border }, 0.2)
					refreshCanvas()
				end
				...
				function Dropfunc:Clear()
					...
					selected = nil; updateText(); refreshCanvas()
				end
				function Dropfunc:Get() return selected end
				function Dropfunc:Set(value, silent)
					if table.find(list, value) then
						selected = value; updateText()
						if not silent then callback(value) end
					end
				end
				function Dropfunc:Add(Text)
					local item = makeDropItem(Scroll, Text, selected == Text)
					item.MouseButton1Click:Connect(function()
						selected = Text; updateText(); callback(Text); Dropfunc:TogglePanel(false)
						for _, c in next, Scroll:GetChildren() do
							if c:IsA("TextButton") then
								Tween(c, { TextColor3 = c.Name == tostring(Text) and Accent or TxtMut }, 0.15)
							end
						end
					end)
					refreshCanvas()
				...
				for _, v in next, list do Dropfunc:Add(v) end
				updateText(); refreshCanvas()
			...
			function functionitem:MultiDropdown(Name, list, default, callback)
				...
				if type(default) == "table" then
					for _, v in next, default do
						if table.find(list, v) and not table.find(DropTable, v) then
				...
				local function updateText()
					if #DropTable > 0 then
						if #DropTable <= 3 then
							DisplayText.Text = Name .. "  ›  " .. table.concat(DropTable, ", ")
						else
							DisplayText.Text = Name .. "  ›  " .. table.concat({DropTable[1], DropTable[2], DropTable[3]}, ", ") .. "  +" .. (#DropTable - 3)
						end
						DisplayText.TextColor3 = TxtPri
					else
						DisplayText.Text = Name; DisplayText.TextColor3 = TxtMut
					end
					fitText(DisplayText, DisplayText.Text, 12, 9)
				end
				task.defer(function()
				...
				function Dropfunc:Clear()
					...
					table.clear(DropTable); updateText(); refreshCanvas()
				end
				function Dropfunc:Get() return DropTable end
				function Dropfunc:Set(values, silent)
					table.clear(DropTable)
					...
					updateText()
					if not silent then callback(DropTable) end
				end
				function Dropfunc:Add(Text)
					local item = makeDropItem(Scroll, Text, table.find(DropTable, Text) ~= nil)
					...
					Check.TextTransparency = table.find(DropTable, Text) and 0 or 1
					...
					item.MouseButton1Click:Connect(function()
						...
						if not found then
							...
							Tween(item, { TextColor3 = Accent }, 0.15)
						else
							table.remove(DropTable, found)
							Tween(Check, { TextTransparency = 1 }, 0.15)
							Tween(item, { TextColor3 = TxtMut }, 0.15)
						end
			...
			function functionitem:Slider(text, floor, min, max, de, callback)
				...
				min = min or 0; max = max or 100; de = de or min
				callback = callback or function() end

				local SliderFrame = Instance.new("Frame")
				SliderFrame.Name = "SliderFrame"
				SliderFrame.Parent = SectionContainer
				SliderFrame.BackgroundColor3 = Raised
				...
				LabelNameSlider.Position = UDim2.new(0, 12, 0, 8)
				LabelNameSlider.Size = UDim2.new(0.6, 0, 0, 13)
				LabelNameSlider.Font = Enum.Font.Gotham
				LabelNameSlider.Text = tostring(text)
				LabelNameSlider.TextColor3 = TxtMut
				LabelNameSlider.TextSize = 10
				LabelNameSlider.TextXAlignment = Enum.TextXAlignment.Left

				-- Value box (top-right)
				local ShowValueFrame = Instance.new("Frame")
				ShowValueFrame.Parent = SliderFrame
				ShowValueFrame.BackgroundColor3 = BG
				ShowValueFrame.BorderSizePixel = 0
				ShowValueFrame.Position = UDim2.new(1, -46, 0, 7)
				ShowValueFrame.Size = UDim2.new(0, 38, 0, 16)
				local SVC = Instance.new("UICorner")
				SVC.CornerRadius = UDim.new(0, 4)
				SVC.Parent = ShowValueFrame
				...
				CustomValue.TextColor3 = Accent
				CustomValue.TextSize = 10

				-- Track
				local ValueFrame = Instance.new("Frame")
				ValueFrame.Parent = SliderFrame
				ValueFrame.BackgroundColor3 = BG
				ValueFrame.BorderSizePixel = 0
				ValueFrame.Position = UDim2.new(0, 12, 0, 32)
				ValueFrame.Size = UDim2.new(1, -24, 0, 3)
				local VC = Instance.new("UICorner")
				VC.CornerRadius = UDim.new(1, 0)
				VC.Parent = ValueFrame

				local MainValue = Instance.new("Frame")
				MainValue.Parent = ValueFrame
				MainValue.BackgroundColor3 = Accent
				MainValue.BorderSizePixel = 0
				MainValue.Size = UDim2.new(0, 0, 1, 0)
				local MVC = Instance.new("UICorner")
				MVC.CornerRadius = UDim.new(1, 0)
				MVC.Parent = MainValue

				local Knob = Instance.new("Frame")
				Knob.Parent = MainValue
				...
				local function formatValue(val)
					return floor and tostring(math.floor(val + 0.5)) or tostring(math.floor(val))
				end
				local function updateVisual(value)
					local clamped = math.clamp(value, min, max)
					local scale = (clamped - min) / (max - min)
					MainValue:TweenSize(UDim2.new(scale, 0, 1, 0), Enum.EasingDirection.Out, Enum.EasingStyle.Quart, 0.15, true)
				end

				CustomValue.Text = formatValue(de)
				updateVisual(de)
				...
				local function move(input)
					...
					CustomValue.Text = formatValue(value)
					updateVisual(value)
					callback(math.clamp(value, min, max))
				end

				Knob.InputBegan:Connect(function(input)
					if input.UserInputType == Enum.UserInputType.MouseButton1 then
						dragging = true
						Tween(SFStroke, { Color = Accent }, 0.15)
					end
				end)
				UserInputService.InputEnded:Connect(function(input)
					if input.UserInputType == Enum.UserInputType.MouseButton1 then
						dragging = false
						Tween(SFStroke, { Color = Border }, 0.15)
					end
				end)
				...
				CustomValue.FocusLost:Connect(function()
					local num = tonumber(CustomValue.Text) or de
					num = math.clamp(num, min, max)
					CustomValue.Text = formatValue(num)
					updateVisual(num)
					callback(num)
				end)
				...
				function sliderfunc:Set(v)
					...
					CustomValue.Text = formatValue(v)
					updateVisual(v)
					callback(v)
				end
				return sliderfunc
...
_G.SelectWeapon = "Melee"
  local WeaponDropdown = AutoFarm:Dropdown("Select Weapons",{"Melee","Sword","Gun","Blox Fruit"},{"Melee"},function(v)
    _G.SelectWeapon = v
     end)
  task.delay(0.1, function()
      if WeaponDropdown and WeaponDropdown.Set then
          WeaponDropdown:Set("Melee")
      end
  end)
    ...
    task.spawn(function()
        while wait(0.5) do
	    	pcall(function()
                ...
                -- Store original category selection
                local selectedCategory = _G.SelectWeapon
                
                if selectedCategory == "Melee" then
                ...
                elseif selectedCategory == "Sword" then
                    for i, v in pairs(Backpack:GetChildren()) do
                ...
                elseif selectedCategory == "Gun" then
                    for i, v in pairs(Backpack:GetChildren()) do
                ...
                elseif selectedCategory == "Fruit" or selectedCategory == "Blox Fruit" then
                    for i, v in pairs(Backpack:GetChildren()) do
...
spawn(function()
    while task.wait() do
        pcall(function()
            -- Only run if auto farm and bring mob are enabled
            if _G.AutoFarm and _G.BringMonster and StartBring then
                -- REFRESH QUEST DATA EVERY LOOP to get current monster for new levels
                CheckQuest()
                
                -- UPDATE PosMon to the CURRENT target monster's position (not old one)
                ...
                -- If we found the current quest monster, update PosMon to ITS live position
                if currentTarget then
                    PosMon = currentTarget.HumanoidRootPart.CFrame
                end
                
                -- CRITICAL FIX: Update PosMon to CURRENT monster's position, not old one
                -- Find the current target monster and update PosMon live
                local currentTarget = nil
                local playerPos = game.Players.LocalPlayer.Character.HumanoidRootPart.Position
                ...
                -- If we found the current quest monster, update PosMon to ITS position
                if currentTarget then
                    PosMon = currentTarget.HumanoidRootPart.CFrame
                end
                
                -- Now bring all valid mobs to the UPDATED PosMon
                if PosMon then
                    for i, v in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                        local isValid = (v.Name == Mon)
                        ...
                        local inRange = (v.HumanoidRootPart.Position - playerPos).Magnitude <= 350
...
spawn(function()
    while wait() do
        pcall(function()
            -- REFRESH quest data so MonFarm updates on level up
            if _G.AutoFarm and _G.BringMob and bringmob then
                CheckQuest()
            end
            
            for i, v in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                if _G.AutoFarm and _G.BringMob and bringmob then
                    -- Use Mon (from CheckQuest) instead of MonFarm for consistency, or keep MonFarm but refresh it
                    if (v.Name == MonFarm or v.Name == Mon) and v:FindFirstChild("Humanoid") and v.Humanoid.Health > 0 then
                        if v.Name == "Factory Staff" then
...
task.spawn(function()
    while task.wait() do
        pcall(function()
            -- Refresh quest data to update Mon and NameMon on level up
            if _G.AutoFarm then
                CheckQuest()
            end
            
            if _G.AutoFarm and PosMon then
                -- Update PosMon to current monster position if we have a live target
                local playerPos = game.Players.LocalPlayer.Character.HumanoidRootPart.Position
                for _, enemy in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                    if enemy.Name == Mon and enemy:FindFirstChild("HumanoidRootPart") and enemy:FindFirstChild("Humanoid") and enemy.Humanoid.Health > 0 then
...
local PointStats = 1 -- Points to add per loop (1 = safest, 50 = faster)

Status:Toggle("Melee", false, function(value)
    melee = value
    -- Turn off other stats when this one is enabled
    if value then
        defense = false
        sword = false
        gun = false
